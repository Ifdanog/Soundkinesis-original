...
function Step2() {
  const [stageNameMsg, setStageNameMsg] = useState('')

  const navigate = useNavigate();

    function formSubmit(e) {
       navigate('/become-an-artist/step3')
        e.preventDefault();
    }

    const validateStageName = () => {
      const stage_name = document.getElementById('stageName')
      const re = /^[a-zA-Z]{2,100}$/

      if(!re.test(stage_name.value)) {
          setStageNameMsg('Invalid name format')
      } else {
        setStageNameMsg('')
        document.cookie = `stage_name=${stage_name.value}; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/`;
      }
    }
    ...
    <div className='bg-gradient-to-r from-purple to-pink h-fit pt-0 md:pt-8 pb-24'>
      <div className="relative">
          <div className="bg-white dark:bg-black py-10 px-6 md:px-10 rounded-none md:rounded-2xl w-full md:w-3/4 mx-auto mt-0 md:mt-2 text-center text-black">
                <p className="align-center dark:text-white">Step 2 of 4</p>
                <div className="w-1/3 mx-auto h-0.5 rounded-xl bg-darker-grey">
                    <div className="w-2/4 h-0.5 bg-gradient-to-r from-purple-900 to-pink-900 rounded-lg"></div>
                </div>
                ...
                <form className="mt-8" onSubmit={formSubmit}>
                  <div className="input-group">
                      <input type="text" id="stageName" className="input my-4" onChange={validateStageName} required />
                      <label htmlFor="stageName" className="label top-0">Stage Name</label>
                      <p className='text-xs text-pink -mt-4'>{stageNameMsg}</p>
                  </div>
                  ...
                  <div className="w-full flex my-8">
                      <button className="text-xs md:text-sm w-full ml-0 text-white py-4 bg-gradient-to-r hover:bg-gradient-to-l from-purple to-pink rounded-xl btn-transt font-medium">Continue</button>
                  </div>
                </form>
            </div>
